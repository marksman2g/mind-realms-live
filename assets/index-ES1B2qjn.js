...
`),L=()=>{let e=l.filter(e=>e.status===`active`),t=l.filter(e=>e.status===`planned`),n=u(),r=e.map(e=>`
    ...
    <section class="screen">
      ...
      <div class="two-column">
        <div class="panel">
          <div class="panel-inner">
            <h3>Primary goal</h3>
            <div class="choice-grid">${ne.map(e=>`<button class="choice-button ${w.primaryGoal===e?`selected`:``}" data-goal="${D(e)}">${D(e)}</button>`).join(``)}</div>
          </div>
        </div>
        <div class="panel">
          <div class="panel-inner">
            <h3>Secondary signal</h3>
            <div class="choice-grid">${re.map(e=>`<button class="choice-button ${w.secondaryGoal===e?`selected`:``}" data-secondary="${D(e)}">${D(e)}</button>`).join(``)}</div>
  ...
  `),B=()=>{let e=v[w.memory.round]??[],t=y.map(e=>`<button class="rune-button" data-rune="${e}">${e}</button>`).join(``),n=`Round ${Math.min(w.memory.round+1,v.length)} of ${v.length}`,r=``;return r=w.memory.phase===`intro`?`
  ...
  `)},V=()=>{let e=b[w.focus.index],t=``;return t=w.focus.phase===`intro`?`
      <div class="trial-surface">
        <div class="panel-inner">
  ...
  `)},H=()=>{let e=x[w.language.index],t=w.language.answers.length;return P(e?`
    <section class="screen">
      <div class="screen-header">
  ...
  `:`
    ...
    `)},U=()=>{let e=S[w.logic.index];return P(e?`
    <section class="screen">
      <div class="screen-header">
  ...
  `)},K=()=>{let e=w.firstQuestComplete?`First quest complete. You improved the path by practicing one strategy immediately.`:`Ori chooses one guided quest from your strongest growth signal. This prototype simulates that first personalized lesson.`;return P(`
    <section class="screen">
      <div class="screen-header">
