    ...
    <div class="app ${E.reducedMotion?`reduced-motion`:``}">
      <header class="topbar">
        ...
        <div class="topbar-actions">
          <button class="ghost-button" data-action="motion">${E.reducedMotion?`Motion off`:`Motion on`}</button>
  ...
  `},I=()=>F(`
    <section class="hero-layout">
      ...
      <div class="scene-panel">
        ...
        <div class="brain-labels" aria-label="Brain region learning hints">
          ${y.map(e=>`
        <button
          type="button"
          class="brain-label ${e.className}"
          data-brain-region="${e.id}"
          aria-label="${k(`${e.label}: ${e.title}. ${e.detail}`)}"
        >
          <span>${k(e.label).replace(` `,`<br />`)}</span>
          <span class="brain-tip" role="tooltip">
      ...
      `).join(``)}
        ...
        <div class="scene-caption">
          <span>${E.bodyForm===`female`?`Female`:`Male`} transparent anatomy frame</span>
          <div class="body-form-toggle" aria-label="Body form">
            <button class="small-button ${E.bodyForm===`female`?`selected`:``}" data-body-form="female">Female</button>
            <button class="small-button ${E.bodyForm===`male`?`selected`:``}" data-body-form="male">Male</button>
          </div>
        </div>
      </div>
    </section>
...
`),R=()=>{let e=l.filter(e=>e.status===`active`),t=l.filter(e=>e.status===`planned`),n=u(),r=e.map(e=>`
      ...
      `).join(``),a=E.topicNotice?`
      <div class="topic-alert ${E.topicNotice.kind}">
        ${E.topicNotice.kind===`duplicate`?`<strong class="flash-word">Already added</strong>`:``}
        <p>${E.topicNotice.message}</p>
        ${E.topicNotice.kind===`duplicate`?`<div class="actions">
    ...
    `:``,o=E.researchQueue.length?`
      <div class="footer-note">
        Research queue: ${E.researchQueue.map(k).join(`, `)}
      </div>
    `:``;return F(`
      ...
      <div class="two-column">
        <div class="panel">
          <div class="panel-inner">
            ...
            <p class="footer-note">${E.audioStatus}</p>
            <p class="audio-transcript">${k(n)}</p>
          </div>
        </div>
        <div class="panel">
          <div class="panel-inner">
            <h3>Add or research a topic</h3>
            <input class="text-input" id="topicDraft" value="${k(E.topicDraft)}" placeholder="Try: focus, sleep, writing, motivation..." />
            <div class="actions">
    ...
    <section class="screen">
      ...
      <div class="two-column">
        <div class="panel">
          <div class="panel-inner">
            <h3>Primary goal</h3>
            <div class="choice-grid">${ne.map(e=>`<button class="choice-button ${E.primaryGoal===e?`selected`:``}" data-goal="${k(e)}">${k(e)}</button>`).join(``)}</div>
          </div>
        </div>
        <div class="panel">
          <div class="panel-inner">
            <h3>Secondary signal</h3>
            <div class="choice-grid">${v.map(e=>`<button class="choice-button ${E.secondaryGoal===e?`selected`:``}" data-secondary="${k(e)}">${k(e)}</button>`).join(``)}</div>
            <textarea class="text-field" id="freeGoal" placeholder="Tell Ori what you want to become.">${k(E.freeGoal)}</textarea>
            <div class="actions">
              <button class="primary-button" data-action="next">${E.primaryGoal?`Open the realms`:`Choose a goal first`}</button>
            </div>
          </div>
        </div>
      </div>
  ...
  `),V=()=>{let e=b[E.memory.round]??[],t=x.map(e=>`<button class="rune-button" data-rune="${e}">${e}</button>`).join(``),n=`Round ${Math.min(E.memory.round+1,b.length)} of ${b.length}`,r=``;return r=E.memory.phase===`intro`?`
    ...
    `:E.memory.phase===`answer`?`
      <div class="trial-surface">
        <div class="panel-inner">
          <p class="feedback">Rebuild the sequence. Current answer: ${E.memory.input.join(` -> `)||`none yet`}</p>
          <div class="answer-row">${t}</div>
        </div>
      </div>
    `:E.memory.phase===`feedback`?`
      <div class="trial-surface">
        <div class="panel-inner">
          <p class="feedback">${E.memory.feedback}</p>
  ...
  `)},H=()=>{let e=S[E.focus.index],t=``;return t=E.focus.phase===`intro`?`
      <div class="trial-surface">
        <div class="panel-inner">
    ...
    `:E.focus.phase===`run`&&e?`
      <div class="trial-surface">
        <div class="panel-inner">
          <p class="feedback">${E.focus.index<5?`Tap Gold Star. Let other signs pass.`:E.focus.index<8?`Tap Gold Star. Ignore Cracked Star.`:E.focus.index<11?`Tap Moon.`:`The tower chimed. Now tap Sun.`}</p>
  ...
  `)},U=()=>{let e=C[E.language.index],t=E.language.answers.length;return F(e?`
    <section class="screen">
      <div class="screen-header">
        <div class="eyebrow">Question ${t+1} of ${C.length}</div>
      ...
      <div class="panel">
        <div class="panel-inner trial-stage">
          ...
          <div class="actions">
            ...
            <button class="small-button" data-action="audio">${E.language.audio?`Audio noted`:`Use audio support`}</button>
          </div>
          <div class="question">
            <h3>${e.prompt}</h3>
            <div class="choice-grid">
              ${e.options.map(e=>`<button class="choice-button" data-language="${k(e)}">${k(e)}</button>`).join(``)}
            </div>
          </div>
        </div>
      </div>
  ...
  `:`
    ...
    `)},W=()=>{let e=w[E.logic.index];return F(e?`
    <section class="screen">
      <div class="screen-header">
        <div class="eyebrow">Puzzle ${E.logic.index+1} of ${w.length}</div>
        <h2>Logic Forge</h2>
          ...
          <div class="trial-surface">
            <div class="panel-inner">
              ...
              <div class="logic-grid">
                ${e.options.map(e=>`<button class="choice-button" data-logic="${k(e)}">${k(e)}</button>`).join(``)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="two-column">
        <div class="panel">
          <div class="panel-inner">
            ...
            <div class="choice-grid">
              ${[`Easier than expected`,`About right`,`Hard but possible`,`Frustrating`,`I felt nervous`].map(e=>`<button class="choice-button ${E.reflection.feeling===e?`selected`:``}" data-feeling="${e}">${e}</button>`).join(``)}
            </div>
          </div>
        </div>
        <div class="panel">
          <div class="panel-inner">
            ...
            <div class="choice-grid">
              ${[`Memory`,`Focus`,`Reading`,`Math`,`Not sure`].map(e=>`<button class="choice-button ${E.reflection.strongest===e?`selected`:``}" data-strongest="${e}">${e}</button>`).join(``)}
            </div>
            <div class="actions">
  ...
  `),K=()=>{let t=me(),n=e(pe()),r=n.domains.map(e=>`
        <article class="profile-card">
          <div class="subject-card-top">
            <span class="subject-status">${k(e.level)}</span>
          ...
          <p>${k(e.evidence)}</p>
          <p><strong>Next:</strong> ${k(e.nextStep)}</p>
        </article>
      `).join(``),i=n.timeline.map(e=>`
        <article class="timeline-step">
          <strong>${k(e.window)}</strong>
          <h3>${k(e.title)}</h3>
          <p>${k(e.focus)}</p>
        </article>
      `).join(``);return F(`
    <section class="screen">
      ...
      <div class="panel">
        <div class="panel-inner">
          ...
          <p class="lede">${k(n.weeklyRhythm)}.</p>
          <p class="footer-note">${k(n.confidenceNote)}</p>
        </div>
      </div>
  ...
  `)},q=()=>{let e=E.firstQuestComplete?`First quest complete. You improved the path by practicing one strategy immediately.`:`Ori chooses one guided quest from your strongest growth signal. This prototype simulates that first personalized lesson.`;return F(`
    <section class="screen">
      <div class="screen-header">
      ...
      <div class="panel">
        <div class="panel-inner">
          ...
          <p class="lede">${ye()}</p>
          <div class="actions">
            <button class="primary-button" data-action="complete-quest">${E.firstQuestComplete?`Quest complete`:`Complete guided quest`}</button>
...
`);function Y(){g.querySelectorAll(`[data-action]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.action;if(t===`next`&&A(),t===`reset`&&j(),t===`motion`&&(E.reducedMotion=!E.reducedMotion,O(`reduced_motion_toggled`,{enabled:E.reducedMotion}),$()),t===`go-report`&&re(`report`),t===`memory-start`&&Z(),t===`memory-next-round`&&(E.memory.round+=1,Z()),t===`focus-start`&&ae(),t===`reread`&&(E.language.reread=!0,O(`language_reread_marked`),$()),t===`audio`&&(E.language.audio=!0,O(`audio_support_selected`),$()),t===`speak-subject-summary`&&M(),t===`stop-subject-summary`&&N(),t===`topic-check`&&P(!1),t===`topic-deep`&&P(!0),t===`topic-clear`&&(E.topicNotice=void 0,E.topicDraft=``,$()),t===`complete-quest`&&(E.firstQuestComplete=!0,O(`first_quest_completed`,{path:Q()}),$()),t===`interest`){O(`pricing_interest_clicked`,{price:`$14.99/month`});let e=document.querySelector(`#interestNote`);e&&(e.textContent=`Interest captured for the prototype. Payment is not active yet.`)}})})}function X(){g.querySelectorAll(`[data-goal]`).forEach(e=>{e.addEventListener(`click`,()=>{E.primaryGoal=e.dataset.goal??``,O(`goal_selected`,{primaryGoal:E.primaryGoal}),$()})}),g.querySelectorAll(`[data-secondary]`).forEach(e=>{e.addEventListener(`click`,()=>{E.secondaryGoal=e.dataset.secondary??``,O(`secondary_goal_selected`,{secondaryGoal:E.secondaryGoal}),$()})});let e=g.querySelector(`#freeGoal`);e?.addEventListener(`input`,()=>{E.freeGoal=e.value});let t=g.querySelector(`#topicDraft`);t?.addEventListener(`input`,()=>{E.topicDraft=t.value}),g.querySelectorAll(`[data-body-form]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.bodyForm;t!==`male`&&t!==`female`||(E.bodyForm=t,O(`body_form_selected`,{bodyForm:t}),$())})});let n=Array.from(g.querySelectorAll(`[data-brain-region]`)),r=()=>{n.forEach(e=>e.classList.remove(`is-active`))};n.forEach(e=>{let t=()=>{r(),e.classList.add(`is-active`)};e.addEventListener(`pointerenter`,t),e.addEventListener(`focus`,t),e.addEventListener(`click`,e=>{e.preventDefault(),t()}),e.addEventListener(`pointerleave`,()=>{e.classList.remove(`is-active`)}),e.addEventListener(`blur`,()=>{e.classList.remove(`is-active`)})}),g.querySelectorAll(`[data-rune]`).forEach(e=>{e.addEventListener(`click`,()=>ie(e.dataset.rune??``))}),g.querySelectorAll(`[data-focus-action]`).forEach(e=>{e.addEventListener(`click`,()=>oe(e.dataset.focusAction===`tap`))}),g.querySelectorAll(`[data-focus-reflect]`).forEach(e=>{e.addEventListener(`click`,()=>{E.focus.reflection=e.dataset.focusReflect??``,E.focus.phase=`done`,O(`focus_pulse_completed`,{missedTargets:E.focus.missedTargets,decoyTaps:E.focus.decoyTaps,switchErrors:E.focus.switchErrors,reflection:E.focus.reflection}),$()})}),g.querySelectorAll(`[data-language]`).forEach(e=>{e.addEventListener(`click`,()=>se(e.dataset.language??``))}),g.querySelectorAll(`[data-logic]`).forEach(e=>{e.addEventListener(`click`,()=>ce(e.dataset.logic??``))}),g.querySelectorAll(`[data-feeling]`).forEach(e=>{e.addEventListener(`click`,()=>{E.reflection.feeling=e.dataset.feeling??``,O(`reflection_feeling_selected`,{feeling:E.reflection.feeling}),$()})}),g.querySelectorAll(`[data-strongest]`).forEach(e=>{e.addEventListener(`click`,()=>{E.reflection.strongest=e.dataset.strongest??``,O(`reflection_strength_selected`,{strongest:E.reflection.strongest}),$()})})}function Z(){E.memory.phase=`show`,E.memory.input=[],E.memory.feedback=``,O(`trial_started`,{trial:`memory`,round:E.memory.round+1}),$(),window.setTimeout(()=>{E.screen===`memory`&&E.memory.phase===`show`&&(E.memory.phase=`answer`,$())},E.reducedMotion?500:1600)}function ie(e){if(E.memory.phase!==`answer`)return;let t=b[E.memory.round]??[];if(E.memory.input.push(e),E.memory.input.length>=t.length){let e=0;t.forEach((t,n)=>{E.memory.input[n]===t?e+=1:E.memory.errorPositions.push(n+1)});let n=t.length-e;E.memory.correctPositions+=e,E.memory.totalPositions+=t.length,E.memory.round===1&&(E.memory.distractionErrors+=n),E.memory.round===2&&(E.memory.delayErrors+=n),O(`memory_sequence_completed`,{round:E.memory.round+1,correct:e,total:t.length}),E.memory.round>=b.length-1?E.memory.phase=`done`:(E.memory.phase=`feedback`,E.memory.feedback=n===0?`The sequence held cleanly. Ori will increase the load gently.`:`Part of the symbol chain faded. Ori is watching where the slip happened.`)}$()}function ae(){E.focus.phase=`run`,E.focus.index=0,E.focus.presentedAt=Date.now(),O(`trial_started`,{trial:`focus`}),$()}function oe(e){if(E.focus.phase!==`run`)return;let t=S[E.focus.index];if(!t)return;let n=t.target===e;E.focus.total+=1,n&&(E.focus.correct+=1),t.target&&!e&&(E.focus.missedTargets+=1),!t.target&&e&&(E.focus.decoyTaps+=1),t.afterSwitch&&!n&&(E.focus.switchErrors+=1),e&&E.focus.responseTimes.push(Date.now()-E.focus.presentedAt),E.focus.index+=1,E.focus.index>=S.length?E.focus.phase=`reflect`:E.focus.presentedAt=Date.now(),$()}function se(e){let t=C[E.language.index];if(!t)return;let n=e===t.answer;E.language.answers.push({id:t.id,correct:n}),E.language.index+=1,O(`language_choice_made`,{id:t.id,correct:n}),$()}function ce(e){let t=w[E.logic.index];if(!t)return;let n=e===t.answer;E.logic.answers.push({id:t.id,correct:n}),E.logic.index+=1,O(`logic_puzzle_completed`,{id:t.id,correct:n}),$()}function le(){return(E.memory.totalPositions?E.memory.correctPositions/E.memory.totalPositions:0)>=.86?`Memory signal: your symbol recall held across load and delay.`:E.memory.distractionErrors>E.memory.delayErrors?`Memory signal: recall shifted most when the vault distracted you.`:E.memory.delayErrors>0?`Memory signal: first recall appeared stronger than delayed recall.`:`Memory signal: Ori needs more evidence, but sequence load should be trained gently.`}function ue(){return E.focus.switchErrors>=2?`Focus signal: attention held early, but the rule switch pulled you back to the old pattern.`:E.focus.decoyTaps>=2?`Focus signal: action speed is strong. The next step is protecting accuracy before tapping.`:E.focus.missedTargets>=2?`Focus signal: some targets slipped during the pulse. Signal detection and re-centering should be trained.`:`Focus signal: your attention held through the pulse. Ori will check endurance over longer sessions later.`}function de(){let e=E.language.answers.some(e=>e.id===`inference`&&!e.correct),t=E.language.answers.filter(e=>e.correct).length;return e?`Reading signal: direct details were clearer than hidden meaning. Inference should be trained next.`:t>=4?`Reading signal: direct detail, sequence, inference, and main idea all held in this short passage.`:`Reading signal: Ori needs more evidence across vocabulary, inference, and retention.`}function fe(){let e=E.logic.answers.filter(e=>!e.correct);return e.some(e=>e.id===`fraction`)?`Logic signal: patterns and balance may be ahead of visual fraction intuition.`:e.some(e=>e.id===`balance`)?`Logic signal: number patterns are waking up. Balance reasoning needs support.`:e.length===0?`Logic signal: pattern and balance reasoning were strong in this first forge run.`:`Logic signal: Ori will separate concept gaps from speed, reading load, and working memory.`}function pe(){let e=E.language.answers.filter(e=>e.correct).length,t=E.logic.answers.filter(e=>e.correct).length;return{goal:E.primaryGoal,secondaryGoal:E.secondaryGoal,freeGoal:E.freeGoal,reflectionFeeling:E.reflection.feeling,reflectionStrongest:E.reflection.strongest,memory:{correctPositions:E.memory.correctPositions,totalPositions:E.memory.totalPositions,delayErrors:E.memory.delayErrors,distractionErrors:E.memory.distractionErrors,errorPositions:E.memory.errorPositions},focus:{correct:E.focus.correct,total:E.focus.total,missedTargets:E.focus.missedTargets,decoyTaps:E.focus.decoyTaps,switchErrors:E.focus.switchErrors,responseTimes:E.focus.responseTimes},reading:{correct:e,total:E.language.answers.length,missedInference:E.language.answers.some(e=>e.id===`inference`&&!e.correct),reread:E.language.reread,audio:E.language.audio},math:{correct:t,total:E.logic.answers.length,missedFraction:E.logic.answers.some(e=>e.id===`fraction`&&!e.correct),missedBalance:E.logic.answers.some(e=>e.id===`balance`&&!e.correct)}}}function me(){return[{label:`Goal`,value:E.primaryGoal||`Become a super learner`},{label:`Memory`,value:le().replace(`Memory signal: `,``)},{label:`Focus`,value:ue().replace(`Focus signal: `,``)},{label:`Reading`,value:de().replace(`Reading signal: `,``)},{label:`Logic`,value:fe().replace(`Logic signal: `,``)},{label:`Strategy`,value:he()},{label:`First path`,value:Q()},{label:`Daily time`,value:`12-15 minutes to start`}]}function he(){if(!E.reflection.strongest)return`Needs one reflection check.`;let e=_e(),t=Math.max(...Object.values(e)),n=e[E.reflection.strongest];if(n!==void 0&&t-n<=.05)return`Your self-read matched your strongest signal today.`;let r=ge();return`You felt strongest in ${E.reflection.strongest}, while Ori saw the strongest signal in ${r}.`}function ge(){let e=_e();return[[`Memory`,e.Memory],[`Focus`,e.Focus],[`Reading`,e.Reading],[`Math`,e.Math]].reduce((e,t)=>t[1]>e[1]?t:e)[0]}function _e(){return{Memory:E.memory.totalPositions?E.memory.correctPositions/E.memory.totalPositions:0,Focus:E.focus.total?E.focus.correct/E.focus.total:0,Reading:E.language.answers.length?E.language.answers.filter(e=>e.correct).length/E.language.answers.length:0,Math:E.logic.answers.length?E.logic.answers.filter(e=>e.correct).length/E.logic.answers.length:0}}function Q(){return E.focus.switchErrors>=2||E.focus.decoyTaps>=2?`Focus Recovery + Memory Anchors + Inference Clues`:E.memory.delayErrors>0||E.memory.distractionErrors>0?`Memory Anchors + Focus Pulse + Logic Patterns`:E.language.answers.some(e=>e.id===`inference`&&!e.correct)?`Inference Clues + Delayed Recall + Logic Patterns`:E.logic.answers.some(e=>!e.correct)?`Logic Forge Basics + Pace Control + Reading Clues`:`Advanced Recall + Flexible Focus + Transfer Quests`}function ve(){let e=Q();return e.startsWith(`Focus`)?`Steady Signal Quest`:e.startsWith(`Memory`)?`Memory Anchor Quest`:e.startsWith(`Inference`)?`Hidden Clue Quest`:e.startsWith(`Logic`)?`Balance Gate Quest`:`Transfer Trial Quest`}function ye(){let e=Q();return e.startsWith(`Focus`)?`You practice one breath before action, then repeat a shorter signal round where accuracy comes before speed.`:e.startsWith(`Memory`)?`You group symbols into anchors, replay them as a tiny story, then retrieve them after a short delay.`:e.startsWith(`Inference`)?`You compare what the scroll says directly with what it implies, then choose a path from the hidden clue.`:e.startsWith(`Logic`)?`You use a visual balance gate before seeing the number sentence, so the concept lands before notation.`:`You combine recall, focus, reading, and logic in one small mixed challenge.`}async function be(){let e=document.querySelector(`#brainScene`);if(e){D?.(),D=null;try{let t=await te(()=>import(`./three.module-kyxZOpvy.js`),[]);if(!document.body.contains(e))return;let n=new t.WebGLRenderer({alpha:!0,antialias:!0});n.setPixelRatio(Math.min(window.devicePixelRatio,2)),e.appendChild(n.domElement);let r=new t.Scene,i=new t.PerspectiveCamera(38,1,.1,100);i.position.set(0,-.42,7.6);let a=new t.Group;a.position.set(0,.12,0),r.add(a);let o=new t.MeshStandardMaterial({color:8051682,emissive:1192763,roughness:.24,metalness:.02,transparent:!0,opacity:.16,depthWrite:!1}),s=new t.MeshStandardMaterial({color:15190667,emissive:3089424,roughness:.46,metalness:.04,transparent:!0,opacity:.18,depthWrite:!1}),c=new t.MeshStandardMaterial({color:16767352,emissive:16758861,roughness:.24,metalness:.18,transparent:!0,opacity:.84}),l=new t.MeshBasicMaterial({color:14678771,transparent:!0,opacity:.62,depthWrite:!1}),u=new t.MeshBasicMaterial({color:15190667,transparent:!0,opacity:.58,depthWrite:!1}),d=new t.MeshBasicMaterial({color:460041,transparent:!0,opacity:.92,depthTest:!1,depthWrite:!1}),f=new t.MeshBasicMaterial({color:16773311,transparent:!0,opacity:.55,depthTest:!1,depthWrite:!1}),p=new t.MeshBasicMaterial({color:6936750,transparent:!0,opacity:.26,depthWrite:!1}),ee=new t.MeshBasicMaterial({color:6936750,wireframe:!0,transparent:!0,opacity:.16,depthWrite:!1}),m=E.bodyForm===`female`,h=m?.72:.86,g=m?.66:.44,_=m?[.86,.75,.52]:[1.05,.8,.56],ne=m?[.46,.78,.43]:[.68,.82,.48],v=[],y=e=>(v.push(e),a.add(e),e),b=(e,n,r)=>{let i=new t.Vector3(...e),a=new t.Vector3(...n),o=a.clone().sub(i),c=o.length(),l=new t.Mesh(new t.CapsuleGeometry(r,Math.max(.04,c-r*2),10,20),s);return l.position.copy(i.add(a).multiplyScalar(.5)),l.quaternion.setFromUnitVectors(new t.Vector3(0,1,0),o.normalize()),y(l)},x=(e,n,r=.012,i=a,o=42)=>{let s=new t.CatmullRomCurve3(e.map(e=>new t.Vector3(...e))),c=new t.Mesh(new t.TubeGeometry(s,o,r,8,!1),n);return i.add(c),c},S=(e,n,r,i=l,o=.5,s=.01)=>{let c=new t.Mesh(new t.TorusGeometry(o,s,10,96),i);return c.position.set(...e),c.scale.set(...n),c.rotation.set(...r),a.add(c),c},C=new t.Mesh(new t.SphereGeometry(.78,42,28),o);C.position.set(0,-.26,-.18),C.scale.set(..._),y(C);let w=new t.Mesh(new t.CapsuleGeometry(.42,.74,18,26),o);w.position.set(0,-.9,-.18),w.scale.set(...ne),y(w);let T=new t.Mesh(new t.SphereGeometry(.5,36,24),o);T.position.set(0,-1.42,-.14),T.scale.set(g*1.42,.44,.46),y(T);let O=new t.Mesh(new t.CapsuleGeometry(.18,.36,14,18),o);O.position.set(0,.52,-.08),O.scale.set(1,1,.65),y(O);let k=new t.Mesh(new t.SphereGeometry(.62,52,34),o);k.position.set(0,1.25,.02),k.scale.set(1.08,1.14,.9),y(k),b([-h,.22,-.08],[-1.42,.22,0],m?.065:.085),b([-1.42,.22,0],[-2.03,.2,.12],m?.058:.07),b([h,.22,-.08],[1.42,.22,0],m?.065:.085),b([1.42,.22,0],[2.03,.2,.12],m?.058:.07),b([-g*.45,-1.58,-.12],[-.33,-2.42,-.04],m?.085:.105),b([-.33,-2.42,-.04],[-.28,-3.14,.03],m?.064:.078),b([g*.45,-1.58,-.12],[.33,-2.42,-.04],m?.085:.105),b([.33,-2.42,-.04],[.28,-3.14,.03],m?.064:.078);let A=new t.Mesh(new t.SphereGeometry(.18,28,20),s);A.position.set(-2.1,.19,.15),A.scale.set(1.25,.34,.42),y(A);let re=A.clone();re.position.x=2.1,y(re);let j=new t.Mesh(new t.SphereGeometry(.16,24,18),s);j.position.set(-.29,-3.22,.2),j.scale.set(1.22,.34,.62),y(j);let M=j.clone();if(M.position.x=.29,y(M),m){let e=new t.Mesh(new t.SphereGeometry(.65,38,24),s);e.position.set(0,1.22,-.12),e.scale.set(.92,1,.62),y(e)}S([0,1.26,.56],[1.08,1.14,.08],[0,0,0],l,.56,.012),S([-.55,1.25,.12],[.18,.28,.06],[0,Math.PI/2,0],u,.5,.006),S([.55,1.25,.12],[.18,.28,.06],[0,Math.PI/2,0],u,.5,.006),x([[-.35,.92,.6],[-.18,.78,.63],[0,.75,.65],[.18,.78,.63],[.35,.92,.6]],l,.01),x([[.02,1.45,.64],[.12,1.32,.71],[.05,1.19,.66]],u,.008),x([[-2.12,.22,.25],[-1.48,.26,.18],[-h,.25,.1],[-.42,.24,.02],[0,.18,.04],[.42,.24,.02],[h,.25,.1],[1.48,.26,.18],[2.12,.22,.25]],l,.016),x([[-h*.58,.18,.42],[-_[0]*.48,-.38,.46],[-g*.55,-1.17,.42],[-g*.74,-1.44,.34]],l,.014),x([[h*.58,.18,.42],[_[0]*.48,-.38,.46],[g*.55,-1.17,.42],[g*.74,-1.44,.34]],l,.014),x([[-.36,.02,.5],[-.18,-.04,.54],[0,-.06,.55],[.18,-.04,.54],[.36,.02,.5]],u,.008),x([[0,.06,.55],[0,-.35,.57],[0,-.82,.54],[0,-1.16,.48]],u,.008),[-.18,0,.18].forEach(e=>{x([[-.28,-.48+e,.56],[-.1,-.52+e,.59],[.1,-.52+e,.59],[.28,-.48+e,.56]],u,.006)}),x([[-.34,-1.48,.4],[-.45,-2.14,.35],[-.35,-2.86,.3],[-.31,-3.22,.32]],l,.012),x([[.34,-1.48,.4],[.45,-2.14,.35],[.35,-2.86,.3],[.31,-3.22,.32]],l,.012),S([-2.11,.2,.28],[.42,.16,.08],[0,0,0],l,.32,.008),S([2.11,.2,.28],[.42,.16,.08],[0,0,0],l,.32,.008),S([-.31,-3.23,.34],[.48,.16,.08],[0,0,-.05],l,.32,.008),S([.31,-3.23,.34],[.48,.16,.08],[0,0,.05],l,.32,.008),v.forEach(e=>{let n=new t.Mesh(e.geometry,ee);n.position.copy(e.position),n.quaternion.copy(e.quaternion),n.scale.copy(e.scale).multiplyScalar(1.012),a.add(n)});let N=new t.Group;N.position.set(0,1.3,.62),N.rotation.set(-.04,-.18,.02),N.scale.setScalar(.82),a.add(N);let P=new t.Mesh(new t.SphereGeometry(.7,56,36),c),F=new t.Vector3(1.22,.78,.58);P.scale.copy(F),P.material.opacity=.08,N.add(P);let I=[],L=(e,n,r,i,a,o=.035)=>{let s=new t.MeshStandardMaterial({color:e,emissive:n,roughness:.36,metalness:.08,transparent:!0,opacity:.96}),c=new t.Mesh(new t.SphereGeometry(.34,48,30),s);return c.position.set(...r),c.scale.set(...i),N.add(c),I.push({mesh:c,phase:a,pulse:o,baseScale:i}),c};L(15952548,7280446,[-.42,.12,.08],[1.28,.86,.68],0),L(9098063,2975772,[.05,.29,0],[1.12,.75,.62],.7),L(16181340,7694870,[.16,.14,.2],[.38,.92,.34],1.25,.025),L(9402584,3154015,[-.16,-.23,.12],[1.16,.55,.62],1.8),L(6147297,1464426,[.48,.02,-.04],[.78,.73,.58],2.25),L(16749179,7548963,[.31,-.47,-.1],[.72,.38,.46],2.75,.018);let R=new t.Mesh(new t.CapsuleGeometry(.09,.5,14,22),new t.MeshStandardMaterial({color:16756876,emissive:6237730,roughness:.38,metalness:.04,transparent:!0,opacity:.94}));R.position.set(.36,-.72,-.08),R.rotation.z=-.18,N.add(R),I.push({mesh:R,phase:3.2,pulse:.012,baseScale:[1,1,1]});let z=new t.Mesh(new t.SphereGeometry(.12,28,18),new t.MeshStandardMaterial({color:16767352,emissive:8082709,roughness:.28,metalness:.12,transparent:!0,opacity:.95}));z.position.set(-.12,-.2,.44),z.scale.set(1.5,.5,.62),N.add(z),I.push({mesh:z,phase:3.8,pulse:.055,baseScale:[1.5,.5,.62]});let B=new t.Mesh(new t.SphereGeometry(.1,26,18),new t.MeshStandardMaterial({color:6936750,emissive:1530953,roughness:.25,metalness:.14,transparent:!0,opacity:.96}));B.position.set(-.5,.18,.43),B.scale.set(1.15,.82,.62),N.add(B),I.push({mesh:B,phase:4.2,pulse:.065,baseScale:[1.15,.82,.62]});let V=(e,t=d,n=.01,r=36)=>x(e,t,n,N,r);V([[-.76,.02,.29],[-.6,.38,.28],[-.2,.54,.24],[.28,.48,.17],[.68,.18,.1],[.64,-.12,.12],[.36,-.35,.16],[-.08,-.42,.24],[-.52,-.24,.3],[-.76,.02,.29]],f,.01,60),V([[.06,.52,.31],[.02,.32,.36],[.04,.12,.38],[.1,-.05,.36]],d,.011),V([[-.72,.27,.37],[-.52,.39,.41],[-.28,.32,.42],[-.14,.44,.39]]),V([[-.72,.08,.42],[-.5,.18,.45],[-.26,.1,.46],[-.08,.2,.43]]),V([[-.65,-.1,.4],[-.42,-.02,.46],[-.2,-.08,.48],[-.02,.03,.44]]),V([[-.52,-.29,.36],[-.26,-.22,.44],[0,-.28,.43],[.18,-.18,.35]]),V([[-.22,.5,.33],[.02,.4,.38],[.24,.44,.3],[.44,.3,.24]]),V([[.1,.29,.42],[.28,.23,.43],[.44,.14,.35],[.6,.03,.24]]),V([[.18,.02,.42],[.36,-.02,.4],[.5,-.13,.28],[.57,-.24,.16]]),V([[-.52,-.31,.3],[-.3,-.39,.37],[-.05,-.37,.36],[.18,-.43,.25]]),[-.16,-.08,0,.08].forEach(e=>{V([[.04,-.47+e,.27],[.24,-.43+e,.26],[.48,-.45+e,.12]],d,.006,22)});let H=new t.Mesh(new t.TorusGeometry(1.22,.008,12,110),p);H.position.set(-.2,.02,.42),H.rotation.set(Math.PI/2.35,.18,0),a.add(H);let U=H.clone();U.position.x=.2,U.rotation.y=-.18,a.add(U);let W=new t.Mesh(new t.TorusGeometry(1.72,.008,12,150),p);W.rotation.x=Math.PI/2,a.add(W);let G=new t.BufferGeometry,K=new Float32Array(540);for(let e=0;e<K.length;e+=3)K[e]=(Math.random()-.5)*4.8,K[e+1]=(Math.random()-.5)*4.2,K[e+2]=(Math.random()-.5)*3.4;G.setAttribute(`position`,new t.BufferAttribute(K,3));let q=new t.Points(G,new t.PointsMaterial({color:15190667,size:.018,transparent:!0,opacity:.58}));r.add(q),r.add(new t.AmbientLight(14602926,1.5));let J=new t.PointLight(16767352,4.2,10);J.position.set(2.6,3.2,3.4),r.add(J);let Y=new t.PointLight(6936750,2.3,9);Y.position.set(-2.8,.4,2.2),r.add(Y);let X=()=>{let t=e.getBoundingClientRect(),r=Math.max(1,t.width),a=Math.max(1,t.height);n.setSize(r,a,!1),i.aspect=r/a,i.updateProjectionMatrix()},Z=0,ie=new ResizeObserver(()=>{window.cancelAnimationFrame(Z),Z=window.requestAnimationFrame(X)});ie.observe(e),X();let ae=0,oe=!0,se=0,ce=(e=0)=>{if(!oe)return;se||=e;let t=e-se;if(!E.reducedMotion){let n=Math.sin(t*45e-5)*.58;a.rotation.y=n,N.rotation.y=-.18+Math.sin(t*6e-4+.8)*.08,P.scale.copy(F).multiplyScalar(1+Math.sin(e*.0022)*.035),I.forEach(({mesh:t,baseScale:n,phase:r,pulse:i})=>{let a=1+Math.sin(e*.0024+r)*i;t.scale.set(n[0]*a,n[1]*a,n[2]*a)}),H.rotation.z+=.003,U.rotation.z-=.003,W.position.y=-1.04+(Math.sin(e*.0017)+1)/2*2.55,q.rotation.y-=.0015}n.render(r,i),ae=window.requestAnimationFrame(ce)};ce(),D=()=>{oe=!1,window.cancelAnimationFrame(ae),window.cancelAnimationFrame(Z),ie.disconnect(),n.dispose(),G.dispose(),e.innerHTML=``}}catch{e.innerHTML=`
      <div class="scene-fallback">
        <div class="fallback-body" aria-label="Symbolic body and mind map"></div>
      </div>
    `}}}function $(){D&&=(D(),null);let e={intro:I,ori:L,subjects:R,goals:z,realms:B,memory:V,focus:H,language:U,logic:W,reflection:G,report:K,quest:q,path:J}[E.screen];g.innerHTML=e(),Y(),X(),E.screen===`intro`&&be()}O(`onboarding_started`),$();